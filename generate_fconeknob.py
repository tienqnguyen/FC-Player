...
export function FcOneKnobPro({ onClose, isOpen = true, audioCtx, inputNode, outputNode }: FcOneKnobProProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/80 backdrop-blur-md">
      ...
      <div className="relative w-full max-w-[1100px] bg-[#1a1b23] rounded-2xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col font-sans">
        
        {/* Top Header */}
        <div className="flex items-center justify-between px-6 py-4 bg-[#121319] border-b border-white/5 relative">
          ...
          <div className="flex items-center gap-3">
            <h2 className="text-white font-black tracking-widest text-2xl flex items-center gap-2">
          ...
          <div className="flex items-center gap-5">
            ...
            <button 
              ...
              className={`px-8 py-2 rounded-md text-xs font-black tracking-widest transition-all border ${!bypass ? 'bg-black/50 text-white/50 border-white/10 hover:text-white hover:border-white/30' : 'bg-orange-500/20 text-orange-400 border-orange-500/50 shadow-[0_0_20px_rgba(249,115,22,0.3)]'}`}
            >
        ...
        {/* Knobs Grid */}
        <div className={`p-8 grid grid-cols-2 md:grid-cols-4 gap-6 bg-[#21232d] ${bypass ? 'opacity-40 grayscale-[0.8] pointer-events-none transition-all duration-500' : 'transition-all duration-500'}`}>
