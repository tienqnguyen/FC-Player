    ...
    <div className="min-h-[100dvh] bg-[#0A0B10] text-[#E0E2E8] font-sans flex flex-col relative overflow-hidden" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      >
        <header className="flex justify-between items-center mb-6 pt-2">
            <h1 className="text-[18px] font-bold tracking-widest uppercase text-white/90 mx-auto">Music Player</h1>
        </header>
        ...
        {recentSongs.length > 0 && (
          <div className="flex-1 flex flex-col items-center justify-center min-h-[350px] bg-[#12141D] p-6 rounded-3xl border border-white/5 mb-6 shadow-xl relative isolate shrink-0">
            ...
            <div className="text-center mb-6 w-full px-2">
                 ...
                 <p className="text-[14px] text-gray-400 font-semibold tracking-wide truncate mt-1 break-all w-full">{currentSong?.author || "Local Studio"}</p>
            </div>
            ...
            <div className="flex items-center gap-6 md:gap-8 justify-center w-full">
                ...
                <button 
                  ...
                  className={\`flex flex-col items-center justify-center p-2 rounded-xl transition-all active:scale-95 \${isSignatureSound ? 'text-[#0A84FF]' : 'text-gray-500 hover:text-white'}\`}
                  title="Toggle Lossless HD Audio"
                >
                    <Zap className={\`w-6 h-6 \${isSignatureSound ? 'fill-[#0A84FF]' : ''}\`} />
                    <span className="text-[9px] font-black uppercase tracking-wider mt-1 border border-current rounded-full px-1.5 py-0.5">HD</span>
                </button>
...
if (startIndex !== -1) {
  ...
  console.log("Could not find the UI block to replace");
}
