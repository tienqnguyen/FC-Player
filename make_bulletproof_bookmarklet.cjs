...
// Completely minify the bookmarklet to be 100% safe
// Replace all newlines and multiple spaces with a single space
let minifiedJS = rawJS.replace(/\n\s*/g, ' ').trim(); 
let bookmarkletURL = "javascript:" + encodeURIComponent(minifiedJS);

// Ensure SNIPPET keeps its readable form for the textarea
...
console.log("Bulletproof Bookmarklet Updated!");
